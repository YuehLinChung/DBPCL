...
def main(args):
    ...
    if args.method == 'ann':
        ...
        print('Top-1: %6.3f'%(topk(y_true, outputs, k=1)*100))
        print('Top-5: %6.3f'%(topk(y_true, outputs, k=5)*100))
    elif args.method in ['knn', 'svm']:
        if args.method == 'knn':
            model = KNN(n_neighbors=args.k, n_jobs=-1)
        elif args.method == 'svm':
            model = SVC(kernel='linear', C=args.c)
        feats_train = []
        labels_train = []
        feats_val = []
        labels_val = []
...
if __name__ == '__main__':
    ...
    parser.add_argument("--method", choices=["knn", "ann", "svm"], default="ann", type=str, help="choose evaluation method from one of: {K Nearest Neighbor, Artificial Neural Network, Support Vector Machine}")
